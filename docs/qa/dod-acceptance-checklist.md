...
> **Status:** static gates (DoD-8, DoD-9) verified from source in this pass (see
> [Static pre-verification](#static-pre-verification)). The interactive gates require the app
> running from a **freshly built** stack — the run-through below is ready to execute and tick off.
> Automated coverage that already backs each gate is linked in the table.
...
- [ ] AC-1 confirmed on a fresh boot.

---

## AC-2 — DoD-1..DoD-10 walk-through

| DoD | Gate | Steps (via the app) | Expected evidence | Automated coverage | ☐ |
|-----|------|---------------------|-------------------|--------------------|----|
...
| **DoD-2** | Teams & epics managed via UI + persist | `/teams`: create/rename/delete a team. `/epics`: pick team, create/edit/delete an epic. Reload. | Rows persist across reload; delete is blocked (disabled/409) while referenced. | `Team*`/`Epic*` unit+integration; `TeamsPage`/`EpicsPage` component tests | ☐ |
...
| **DoD-4** | Comments with author + timestamp | On a ticket, add a comment. | Comment lists oldest-first with author email + timestamp; author may edit/delete own (shows "(edited)"). | `Comment*IntegrationTest`, `CommentServiceTest`; `CommentsPanel`/`comment-edit` tests | ☐ |
| **DoD-5** | Board shows tickets in correct state columns per team | `/board`: select a team; create tickets in various states. | Five workflow-ordered columns; each card under its state; counts correct; team switch reloads. | `BoardQueryIntegrationTest`; `BoardPage`/`virtualized-board` tests | ☐ |
...
| **DoD-7** | Starts from clean checkout with `docker compose up --build` | From a clean clone + `.env` (copy of `.env.example`), run the compose command. | All four services healthy; app reachable at :8081. | `docker-compose.yml` + per-service Dockerfiles present; CI builds both (`.github/workflows/ci.yml`) | ☐ |
...
| **DoD-9** | Fresh DB = schema + migration metadata only | The AC-1 procedure above. | Zero application rows on first boot. | **Verified statically — see below** (no seed data exists) | ✅ (static) |
| **DoD-10** | All test/demo data creatable via UI/API (no manual DB edits) | Perform DoD-2..DoD-6 entirely through the UI. | Every entity (team, epic, ticket, comment, state change) is created through the app; DB is never edited by hand. | Covered by the DoD-2..6 flows above | ☐ |
...
- [ ] AC-3 confirmed (no manual DB edits were needed to complete the walk-through).
...
- [ ] AC-4 — failures filed & linked (or "no defects").

**Filed defects:** _none yet_.
...
- [ ] AC-1..AC-4 met
- [ ] DoD-1..DoD-10 ticked with evidence
- [ ] Fresh-DB emptiness confirmed on a clean boot

_Run-through performed by: ______  ·  Date: ______  ·  Build/commit: _______
